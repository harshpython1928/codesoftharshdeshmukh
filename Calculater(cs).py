
#The following program is of an simple calculator which is made using Tkinter.
#The calculator can perform simple calculus.

from tkinter import *
expression = ""  

# Function to update expression in the text entry box 

def press(num): 
    global expression 
    expression = expression + str(num) 
    equation.set(expression) 
def equalpress(): 
   
    #Try and except statement
    
    try: 
        global expression 
        total = str(eval(expression)) 
        equation.set(total) 
        expression = "" 
    except: 
 
        equation.set(" error ") 
        expression = "" 
        
def clear(): 
    global expression 
    expression = "" 
    equation.set("") 
 
if __name__ == "__main__": 
    # creating a GUI window and set color,title,configuration.
   
    gui = Tk() 
    gui.configure(background="light gray") 
    gui.title("The Calculator") 
    gui.geometry("300x300") 
    equation = StringVar() 
 
    #creating the text entry box for showcasing of equations.
   
    expression_field = Entry(gui, textvariable=equation) 
 
    expression_field.grid(columnspan=4, ipadx=70)
 
    #creating all Buttons that are required for calculators functions.
 
    button1 = Button(gui, text=' 1 ', fg='white', bg='orange', 
                    command=lambda: press(1), height=2, width=6) 
    button1.grid(row=2, column=0) 
 
    button2 = Button(gui, text=' 2 ', fg='white', bg='orange', 
                    command=lambda: press(2), height=2, width=6) 
    button2.grid(row=2, column=1) 
 
    button3 = Button(gui, text=' 3 ', fg='white', bg='orange', 
                    command=lambda: press(3), height=2, width=6) 
    button3.grid(row=2, column=2) 
 
    button4 = Button(gui, text=' 4 ', fg='black', bg='white', 
                    command=lambda: press(4), height=2, width=6) 
    button4.grid(row=3, column=0) 
 
    button5 = Button(gui, text=' 5 ', fg='black', bg='sky blue', 
                    command=lambda: press(5), height=2, width=6) 
    button5.grid(row=3, column=1) 
 
    button6 = Button(gui, text=' 6 ', fg='black', bg='white', 
                    command=lambda: press(6), height=2, width=6) 
    button6.grid(row=3, column=2) 
 
    button7 = Button(gui, text=' 7 ', fg='white', bg='green', 
                    command=lambda: press(7), height=2, width=6) 
    button7.grid(row=4, column=0) 
 
    button8 = Button(gui, text=' 8 ', fg='white', bg='green', 
                    command=lambda: press(8), height=2, width=6) 
    button8.grid(row=4, column=1) 
 
    button9 = Button(gui, text=' 9 ', fg='white', bg='green', 
                    command=lambda: press(9), height=2, width=6) 
    button9.grid(row=4, column=2) 
 
    button0 = Button(gui, text=' AC ', fg='white', bg='black', 
                    command=clear, height=1, width=7) 
    button0.grid(row=6, column=0) 
 
    plus = Button(gui, text=' + ', fg='white', bg='black', 
                command=lambda: press("+"), height=1, width=7) 
    plus.grid(row=2, column=3) 
 
    minus = Button(gui, text=' - ', fg='white', bg='black', 
                command=lambda: press("-"), height=1, width=7) 
    minus.grid(row=3, column=3) 
 
    multiply = Button(gui, text=' * ', fg='white', bg='black', 
                    command=lambda: press("*"), height=1, width=7) 
    multiply.grid(row=4, column=3) 
 
    divide = Button(gui, text=' / ', fg='white', bg='black', 
                    command=lambda: press("/"), height=1, width=7) 
    divide.grid(row=5, column=3) 
 
    equal = Button(gui, text=' = ', fg='white', bg='black', 
                command=equalpress, height=1, width=7) 
    equal.grid(row=6, column=2) 
 
    clear = Button(gui, text='0', fg='white', bg='black', 
                command=lambda: press("0"), height=2, width=6) 
    clear.grid(row=5, column='1') 
 
    Decimal= Button(gui, text='.', fg='white', bg='black', 
                    command=lambda: press('.'), height=1, width=7) 
    Decimal.grid(row=6, column=3) 
    # start the GUI 
    gui.mainloop() 